'''
This main script looks to generate a Cleaned STORM EVENT dataframe, with corrected fips.
'''
...
def fix_fips_codes():
    '''Function that makes corrections in the storm_events fips by reading the information in official county data.
    '''
    # Read data
    county = gpd.read_parquet(COUNTY_DATA_PATH)
    # outages = utils.get_required_outages_dfs(2014, 2015, 2016, eaglei_data_path=EAGLEI_DATA_PATH)
    ...
    storm_events['fips'] = (
            storm_events.STATE_FIPS.astype(str).str.zfill(2) + storm_events.CZ_FIPS.astype(str).str.zfill(3)
    )
    fips_code__storm_events_unique = storm_events['fips'].unique()
    # Calculate unofficial fips on each dataframe.
    #outage_unofficial_fips = len(set(fips_code__outages_unique) - set(fips_code__county_unique))
    #print(f'Outage dataframe contains {outage_unofficial_fips} unofficial fips.')
    storm_events_unofficial_fips = len(set(fips_code__storm_events_unique) - set(fips_code__county_unique))
    print(f'Storm Events dataframe contains {storm_events_unofficial_fips} unofficial fips.')
    print(f'Assumption: These {storm_events_unofficial_fips} unofficial fips might be typos or mistakes.')
    ...
    solved_storm_sol = []
    # For each state in county, we are analyzing the FIPS in one and another dataframe.
    ...
    storm_events_unofficial_fips_new = len(set(fips_code__storm_events_new_unique) - set(fips_code__county_unique))
    print(f'After cleaning, Storm Events dataframe contains {storm_events_unofficial_fips_new} unofficial fips.')
    corrected_fips = storm_events_unofficial_fips - storm_events_unofficial_fips_new
    print(f'Total amount of corrected fips: {corrected_fips}')
    print(f'Saving Storm events cleaned dataframe at: {STORM_EVENTS_CLEANED_PATH}')
