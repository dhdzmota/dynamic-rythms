...
SPLITTED_FILENAMES = ['train', 'test', 'OOT', 'eval', 'cal']

INTERIM_DATA_PATH = utils.get_data_path('interim')

# Splits train 80 and test 20
TEST_SIZE = 0.2
# From the remaining train, splits into a transitory set, which will be converted into cal and eval sets
TRANS_SIZE = 0.2
# Split of the eval and cal to be CAL_EVAL_SIZE  and 1-CAL_EVAL_SIZE, corresponding to TRANSIZE/2 form the remining train
CAL_EVAL_SIZE = 0.5
...
def get_train_and_oot_candidates():
    ...
    print(f'Train window data has {TRAIN_WINDOW_DATA_PERCENTAGE*100}% of data.')
    print(f'OOT window data has {OOT_WINDOW_DATA_PERCENTAGE*100}% of data.')
    print(f'We are discarding {(1-TRAIN_WINDOW_DATA_PERCENTAGE-OOT_WINDOW_DATA_PERCENTAGE)*100}% of data.')
    max_train_date = episode_fips_dates_max[episode_fips_dates_max < TRAIN_WINDOW_DATA_PERCENTAGE].tail(1).index[0]
    min_oot_date = episode_fips_dates_min[episode_fips_dates_min > (1-OOT_WINDOW_DATA_PERCENTAGE)].head(1).index[0]
    # Split the dataframe, get the corresponding episode_fips ids.
    train_window_candidates = episode_fips_dates[episode_fips_dates.max_date < max_train_date].index
    oot_window_candidates = episode_fips_dates[episode_fips_dates.max_date > min_oot_date].index
    print('Done.')
    return data, train_window_candidates, oot_window_candidates


def train_test_cal_eval_splits(train_window_candidates, oot_window_candidates, print_percentages=True):
    ...
    cal_episode_fips_id, eval_episode_fips_id = train_test_split(
        transitory_episode_fips_id, test_size=CAL_EVAL_SIZE, random_state=RANDOM_SEED
    )
    print('Done.')
    datasets_fips_ids = {
...
def splitting_process(save=True, return_data=False):
    data, train_window_candidates, oot_window_candidates = get_train_and_oot_candidates()
